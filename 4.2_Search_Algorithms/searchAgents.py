...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    remaining = [corner for corner, seen in zip(corners, visited) if not seen]
    ...
    # Cost to reach the closest remaining corner
    nearest = min(manhattan(position, corner) for corner in remaining)

    # Minimum spanning tree over remaining corners using Manhattan distances
    def mst_cost(points):
        if len(points) <= 1:
            return 0
        unused = set(points)
        current = unused.pop()
        total = 0
        best = {point: manhattan(current, point) for point in unused}

        while unused:
            next_point = min(unused, key=lambda p: best[p])
            total += best[next_point]
            unused.remove(next_point)
            for point in unused:
                cost = manhattan(next_point, point)
                if cost < best[point]:
                    best[point] = cost
        return total

    return nearest + mst_cost(remaining)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    return max(cached_maze_distance(position, food) for food in food_list)
